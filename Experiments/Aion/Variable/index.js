...
function Aion(canvas) {
	...
	that.ShowMessage = function(Message, Color) {
	...
	that.HalfPeriodCheck = function() {
		if(that.Guessed != 2 && that.Guessed != 4) {
			...
			document.getElementById("TracksLostMessage").innerText = "You were too late.";
			document.getElementById("TracksLostMessage").style.color = "#F44336";
			that.WillStopPlaying();
		}
		if(that.Guessed == 4) {
	...
	that.WillStopPlaying = function() {
		that.Busy = true;
		setTimeout(that.StopPlaying, 5000);
	}
	
	that.StartPlaying = function() {
		that.Period = parseInt(prompt("How many minutes will your task take?", ""));
		if(that.Period >= 60) {
			alert("Too high");
			return;
		}
		if(isNaN(that.Period)) {
			return;
		}
		document.title = "Aion | " + that.Period + " minutes now?";
		if(that.Period == 1) {
			document.title = "Aion | " + that.Period + " minute now?";			
		}
		that.Playing = true;
		that.d0 = new Date();
		...
		that.Guessed = 2;
		that.Busy = false;
		document.getElementById("TracksLostMessage").innerText = "Tap when your task is done";
		document.getElementById("TracksLostMessage").style.color = "#333";
		document.getElementById("TracksLostMessage").style.opacity = 1;
	}
	
	that.StopPlaying = function() {
		that.Playing = false;
		document.getElementById("Message").innerText = "Please tap to start";
	...
	that.FlushData = function(time, secondary) {
		time = "{time:" + time + ", period:" + that.Period + "}";
...
function Post(path, params) {
	...
	path = path || "https://maker.ifttt.com/trigger/aion_discretevar_data/with/key/bBTgZ_uh4v_eh76VIq8MpsTIij3L67rf8ofI8kyaHtv"
